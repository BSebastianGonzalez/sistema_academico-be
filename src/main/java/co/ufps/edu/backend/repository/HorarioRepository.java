...
public interface HorarioRepository extends JpaRepository<Horario, Long> {
    ...
    @Query("SELECT h FROM Horario h WHERE h.curso.id = :cursoId AND h.dia = :dia")
    List<Horario> findByCursoIdAndDia(@Param("cursoId") Long cursoId, @Param("dia") DiaSemana dia);

    @Query("SELECT h FROM Horario h WHERE h.aula = :aula AND h.dia = :dia AND h.tipo = 'EXAMEN'")
    List<Horario> findExamenesByAulaAndDia(@Param("aula") Aula aula, @Param("dia") DiaSemana dia);
